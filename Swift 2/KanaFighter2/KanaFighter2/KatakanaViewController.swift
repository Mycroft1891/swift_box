...
class KatakanaViewController: UIViewController, ADBannerViewDelegate {
    ...
    @IBAction func buttonOne(sender: UIButton) {
        ...
        if checkTrueAnswer(someRandomNumber[0]) == true {
            ...
            guard didPlayerLose(totalLives) == false
                    ...
                    return self.performSegueWithIdentifier("GameoverK", sender: nil)
            }
        }
    }
    ...
    func setMyButtonText(integerArray : [Int]) {
        buttonTextOne.setTitle("\(katakanaRomaji[integerArray[0]])", forState: .Normal)
        buttonTextTwo.setTitle("\(katakanaRomaji[integerArray[1]])", forState: .Normal)
        buttonTextThree.setTitle("\(katakanaRomaji[integerArray[2]])", forState: .Normal)
        buttonTextFour.setTitle("\(katakanaRomaji[integerArray[3]])", forState: .Normal)
    }
