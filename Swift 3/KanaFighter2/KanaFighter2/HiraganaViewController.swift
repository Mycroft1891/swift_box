...
class HiraganaViewController: UIViewController, ADBannerViewDelegate {
    ...
    @IBAction func buttonOne(_ sender: UIButton) {
        ...
        if checkTrueAnswer(someRandomNumber[0]) == true {
            ...
            guard didPlayerLose(totalLives) == false
                    ...
                    return self.performSegue(withIdentifier: "GameoverH", sender: nil)
            }
        } 
    }
    ...
    func setMyButtonText(_ integerArray : [Int]) {
        buttonTextOne.setTitle("\(hiraganaR[integerArray[0]])", for: UIControlState())
        buttonTextTwo.setTitle("\(hiraganaR[integerArray[1]])", for: UIControlState())
        buttonTextThree.setTitle("\(hiraganaR[integerArray[2]])", for: UIControlState())
        buttonTextFour.setTitle("\(hiraganaR[integerArray[3]])", for: UIControlState())
    }
